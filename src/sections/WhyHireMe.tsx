    ...
    <section id="why-hire-me" ref={sectionRef} className="relative overflow-hidden bg-cream-light py-20 md:py-28">
      <div className="mx-auto grid w-full max-w-6xl gap-10 px-6 md:grid-cols-[0.85fr_1.15fr] md:px-12">
        <ScrollReveal blur={false}>
          ...
          <h2 className="font-serif-display text-[clamp(2.4rem,5vw,4.5rem)] font-bold leading-none text-dark">
        ...
        <div className="grid gap-3">
          {siteData.whyHireMe.points.map((point, index) => (
            <ScrollReveal key={point} blur={false} delay={index * 0.04}>
              <div className="flex gap-4 border-b border-dark/10 py-4">
                <span className="font-heading text-2xl font-bold leading-none text-dark/35">
                  {String(index + 1).padStart(2, '0')}
                </span>
                <p className="font-body text-base font-medium leading-relaxed text-dark/80 md:text-lg">
                  {point}
                </p>
              </div>
            </ScrollReveal>
